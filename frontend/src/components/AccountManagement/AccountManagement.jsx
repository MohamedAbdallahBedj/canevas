...
const AccountManagement = ({ data = [], refetch, loading = false }) => {
  const { t } = useTranslation();
  const { user } = React.useContext(AuthContext);
  const { idWilaya: id } = user;
  const fileName = t("Outil de gestion des comptes");
        ...
        editable={{
          ...
          onRowDelete: (oldData) =>
            new Promise((resolve, reject) => {
              fetch("/api/auth/delete/" + oldData.nomUtilisateur, {
                method: "delete",
                credentials: "include",
                headers: {
