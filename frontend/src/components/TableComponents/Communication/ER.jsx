...
const ER = ({ campagne, data = [], refetch, loading = false }) => {
  const { t } = useTranslation();
  const { user } = React.useContext(AuthContext);
  const { idWilaya: id } = user;  const fileName = t("Emissions Radiophoniques");
  ...
  let columns = [
    ...
    },
    {
      ...baseColumn,
      field: "idType",
      title: t("Type d'Emission"),
      lookup: idTypeLookup,
    },
    { ...baseColumn, field: "theme", title: t("Theme") },
  ];
  if (id !== 59)
            ...
            : {
                onRowAdd: (newData) =>
                  new Promise((resolve, reject) => {
                    ...
                    fetch("/api/communication/add", {
                      ...
                      body: JSON.stringify({
                        tablename: "emissionRadiophonique",
                ...
                onRowUpdate: (newData, oldData) =>
                  new Promise((resolve, reject) => {
                    ...
                    const { idEmission, tableData, ...properties } = newData;
                    fetch("/api/communication/edit", {
                      method: "post",
                      credentials: 'include',
                      headers: {
                        "Content-Type": "application/json",
                ...
                onRowDelete: (oldData) =>
                  new Promise((resolve, reject) => {
                    fetch("/api/communication/delete/" + oldData.idEmission, {
                      method: "delete",
                      credentials: 'include',
                      headers: {
                        "Content-Type": "application/json",
