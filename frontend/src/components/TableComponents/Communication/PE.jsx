...
const PE = ({ campagne, data = [], refetch, loading = false }) => {
  const { t } = useTranslation();
  const { user } = React.useContext(AuthContext);
  const { idWilaya: id } = user;  const fileName = t("Presse Ecrite");
  ...
  let columns = [
    ...
    { ...baseColumn, field: "presse", title: t("Presse") },
  ];
  if (id !== 59)
            ...
            : {
                ...
                onRowUpdate: (newData, oldData) =>
                  new Promise((resolve, reject) => {
                    ...
                    const { idPresse, tableData, ...properties } = newData;
                ...
                onRowDelete: (oldData) =>
                  new Promise((resolve, reject) => {
                    fetch("/api/communication/delete/" + oldData.idPresse, {
                      method: "delete",
                      credentials: 'include',
                      headers: {
                        "Content-Type": "application/json",
