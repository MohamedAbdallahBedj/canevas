...
const Bilan = ({ campagne, data = [], refetch, loading = false }) => {
  const { t } = useTranslation();
  const { user } = React.useContext(AuthContext);
  const { idWilaya: id, role } = user;
  const fileName = t("Bilan Production de Mais");
  const idEspeceLookup = { 1: t("Mais"), 2: t("Mais (Fourrage)") };
        ...
        editable={
          id === 59
            ...
            : {
                ...
                onRowDelete: (oldData) =>
                  new Promise((resolve, reject) => {
                    fetch("/api/legumes-secs/delete/" + oldData.idBilan, {
                      method: "delete",
                      credentials: "include",
                      headers: {
                        "Content-Type": "application/json",
