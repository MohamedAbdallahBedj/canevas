...
const Associations = ({ data = [], refetch, loading = false, relation }) => {
  const { t } = useTranslation();
  const { user } = React.useContext(AuthContext);
  const { idWilaya: id } = user;  const fileName = t(
    "Identification d'Associations Professionnelles Agricoles"
  );
        ...
        editable={
          id === 59
            ...
            : {
                ...
                onRowDelete: (oldData) =>
                  new Promise((resolve, reject) => {
                    fetch("/api/associations-cooperatives/delete/" + oldData.idAssociation, {
                      method: "delete",
                      credentials: 'include',
                      headers: {
                        "Content-Type": "application/json",
