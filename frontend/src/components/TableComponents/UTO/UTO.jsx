...
const UTO = ({ campagne, data = [], refetch, loading = false }) => {
  const { t } = useTranslation();
  const { user } = React.useContext(AuthContext);
  const { idWilaya: id } = user;  const fileName = t(
    "Tableau Récapitulatif des Unités de Transformation Oléicoles"
  );
  const idNatureLookup = { 1: t("Huilerie"), 2: t("Confiserie") };
  ...
  let columns = [
    ...
    },
    {
      ...baseColumn,
      field: "idNature",
      lookup: idNatureLookup,
      title: t("Espèces"),
    },
    {
      ...baseColumn,
      field: "nombreFonctionnelle",
      title: t("Nombre Fonctionnelle"),
      type: "numeric",
    },
    {
      ...baseColumn,
      field: "nombreArret",
      title: t("Nombre a l'Arrêt"),
      type: "numeric",
    },
    {
      ...baseColumn,
      field: "capaciteTransformation",
      title: t("Capacité de Transformation (Q/An)"),
      type: "numeric",
    },
    {
      ...baseColumn,
          ...
          exportMenu: [
            {
              ...
              exportFunc: (columns, _, { searchedData }) => {
                ...
                const searchedDataFormated = searchedData.map((row) => ({
                  ...row,
                  idNature: idNatureLookup[row.idNature],
                }));
            ...
            : {
                onRowAdd: (newData) =>
                  new Promise((resolve, reject) => {
                    ...
                    fetch("/api/olives/add", {
                      ...
                      body: JSON.stringify({
                        tablename: "unitesTransformationOleicoles",
                ...
                onRowUpdate: (newData, oldData) =>
                  new Promise((resolve, reject) => {
                    ...
                    const { idUnite, tableData, ...properties } = newData;
                ...
                onRowDelete: (oldData) =>
                  new Promise((resolve, reject) => {
                    fetch("/api/olives/delete/" + oldData.idUnite, {
                      method: "delete",
                      credentials: 'include',
                      headers: {
                        "Content-Type": "application/json",
