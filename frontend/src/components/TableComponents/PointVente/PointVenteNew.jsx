...
const defaultLat = 35.833088178381614;
const defaultLong = 3.157359755620372;
...
const PointVenteNew = () => {
  ...
  const { state: campagne } = useLocation();
  const [products, setProducts] = React.useState([]);
  const [latLong, setLatLong] = React.useState([defaultLat, defaultLong]);
  ...
  const handleProductSubmit = (event) => {
    ...
    setProducts((previous) => [
      ...previous,
      { idProduit, quantiteDisponible, prixVente },
    ]);
    event.target.reset();
    return;
  };
  ...
  if (!campagne || error) return <NotFound />;
