...
const Plasticulture = ({ campagne, data = [], refetch, loading = false }) => {
  const { t } = useTranslation();
  const { user } = React.useContext(AuthContext);
  const { idWilaya: id } = user;  const fileName = t("Suivi de la Production Plasticulture");
  const idTypeSerreLookup = {
    1: t("Serre Tunnel"),
    2: t("Serre Multichapelle"),
    3: t("Serre Canarienne"),
  };
          ...
          exportMenu: [
            {
              ...
              exportFunc: (columns, _, { searchedData }) => {
                ...
                const searchedDataFormated = searchedData.map((row) => ({
                  ...row,
                  idTypeSerre: idTypeSerreLookup[row.idTypeSerre],
        ...
        editable={
          id === 59
            ...
            : {
                ...
                onRowDelete: (oldData) =>
                  new Promise((resolve, reject) => {
                    fetch("/api/plasticulture/delete/" + oldData.idSuivi, {
                      method: "delete",
                      credentials: 'include',
                      headers: {
                        "Content-Type": "application/json",
