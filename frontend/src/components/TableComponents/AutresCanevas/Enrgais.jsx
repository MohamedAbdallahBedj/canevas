...
const Enrgais = ({ campagne, data = [], refetch, loading = false }) => {
  const { t } = useTranslation();
  const { user } = React.useContext(AuthContext);
  const { idWilaya: id } = user;  const fileName = t("Suivi de l'Engrais Utilisés par les Agriculteurs");
  ...
  let columns = [
    ...
    },
    {
      ...baseColumn,
      field: "surface",
      title: t("Surface de l'Exploitation (Ha)"),
      type: "numeric",
    },
    {
      ...baseColumn,
      field: "engraisUtilise",
      title: t("Engaris Utilisés  (Ha)"),
      type: "numeric",
    },
    {
      ...baseColumn,
        ...
        editable={
          id === 59
            ...
            : {
                ...
                onRowDelete: (oldData) =>
                  new Promise((resolve, reject) => {
                    fetch("/api/autres-canevas/delete/" + oldData.id, {
                      method: "delete",
                      credentials: 'include',
                      headers: {
                        "Content-Type": "application/json",
